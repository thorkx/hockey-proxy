...
PRIORITY_CONFIG = {
	"LEAGUES": {
		...
		"f1": 800, "cpl": 700, "plc":700, "nsl": 700, "sln": 700, "cfl": 500, "lcf": 700, "pwhl": 700, "lphf": 700
	},
	"TEAMS": {
		"CANADIENS": 2000,
...
def fetch_pwhl():
	...
	url = 'https://lscluster.hockeytech.com/feed/index.php?feed=modulekit&view=gamesperday&start_date=' + start.strftime('%Y-%m-%d') + '&end_date=' + end.strftime('%Y-%m-%d') + '&key=446521baf8c38984&client_code=pwhl'
	results = []
	print('Fetching PWHL schedule from ' + url)
	try:
		r = requests.get(url, timeout=5)
		r.raise_for_status()
		payload = r.json()
	except Exception as exc:
		print("Skipping PWHL day")
	pprint.pprint(payload.get('events', []))
...
def generate_schedule(days=2):
	...
	leagues = [
		...
		('soccer','nsl'), ('football','cfl'), ('basketball','wnba'), ('football','lcf'), ('hockey','pwhl'), ('hockey','lphf')
	]
	...
	# --- SOURCE 4: PWHL (Précision pour la PWHL) ---
	print('Fetching PWHL events...')
	pwhl_events = fetch_pwhl()
	for e in pwhl_events:
		events_to_process.append({
			'id': e['id'],
			'name': e['name'],
			'date': e['date'],
