...
# Scores de priorité
P_ULTRA = 2500  # Tes favoris absolus
P_HIGH  = 1200  # Champions League, Favoris secondaires
P_STD   = 50    # Le reste du monde sportif
...
CH = {
    # Conventionnels & Sports canadiens
...
# Génération dynamique DAZN (1-100) : 176642 à 176741
for i in range(1, 101):
    CH[f"DAZN{i}"] = str(176642 + (i - 1))

# Génération dynamique MLS Apple (1-50) : 175474 à 175523
for i in range(1, 51):
    CH[f"MLS{i}"] = str(175474 + (i - 1))

def get_url(cid):
    return f"http://{BASE_DOMAIN}/{USER}/{PASS}/{cid}.ts"

# Mapping pour la détection textuelle
MAPPING = {
...
# =================================================================
# 2. LOGIQUE DE RANKING & ATTRIBUTION
# =================================================================
...
def get_ranked_games():
        ...
        for i in range(2):
            try:
                ...
                for e in r.get('events', []):
                    title_upper = e['name'].upper()
                    teams = [c['team']['abbreviation'].upper() for c in e['competitions'][0]['competitors']]
                    
                    score = P_STD
                    if any(t in teams for t in ULTRA_TEAMS) or any(fav in title_upper for fav in ULTRA_TEAMS):
                        score = P_ULTRA
                    elif sport_key == 'F1': score = 1800
                    elif 'champions' in path: score = P_HIGH
    ...
    for item in all_games:
        ...
        for net in item['networks']:
            ...
            elif "DAZN" in mk or "MLS" in mk or "APPLE" in mk:
                bonus = 600
